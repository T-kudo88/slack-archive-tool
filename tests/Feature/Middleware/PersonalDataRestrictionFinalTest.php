...
class PersonalDataRestrictionFinalTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function middleware_allows_access_to_own_messages()
    {
        ...
        Route::middleware(['auth', 'personal.data.restriction'])->get('/test/messages/{id}', function ($id) {
            return response()->json(['accessed_message_id' => $id]);
        });
        ...
        $response->assertJson(['accessed_message_id' => (string)$message->id]);
    }
    ...
    public function middleware_blocks_access_to_other_users_messages()
    {
        ...
        Route::middleware(['auth', 'personal.data.restriction'])->get('/test/messages/{message}', function ($message) {
            return response()->json(['accessed_message_id' => $message]);
        });
    ...
    /** @test */
    public function middleware_logs_admin_access_with_justification()
    {
        $workspace = Workspace::factory()->create();
        $channel = Channel::factory()->create(['workspace_id' => $workspace->id]);

        Route::middleware(['auth', 'personal.data.restriction'])->get('/test/users/{user}', function ($user) {
            return response()->json(['success' => true]);
        });
        ...
        ])->get("/test/users/{$this->regularUser->id}");
