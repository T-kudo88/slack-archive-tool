...
function useForm(rememberKeyOrInitialValues, maybeInitialValues) {
  ...
  const [data, setData] = rememberKey ? useRemember(defaults2, `${rememberKey}:data`) : (0, import_react8.useState)(defaults2);
  ...
  const isDirty = (0, import_react8.useMemo)(() => !isEqual(data, defaults2), [data, defaults2]);
      ...
      if (fields.length === 0) {
        ...
        setData(
          (data2) => fields.filter((key) => has(defaults2, key)).reduce(
            (carry, key) => {
              return set(carry, key, get(defaults2, key));
            },
            { ...data2 }
          )
        );
